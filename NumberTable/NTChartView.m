...
-(void) drawInscriptions:(CGRect)rect context:(CGContextRef) context{
    double nintyPersent = 0.9;
    int horizontalStep = rect.size.width / self.values.count;
    int verticalSize;
    int bottomIndent = 32;
    int lateralIndent = 10;
    long textValue = 100/[self.values count] - 100;
    for (int i = 0; i < self.values.count; i++) {
        verticalSize = ([self.values[i] intValue]*(rect.size.height-bottomIndent)/([self maxCount]))*nintyPersent;
...
-(int) maxCount{
    int res = - 100;
    for (int i = 0;i < self.values.count; i++) {
        if (res<[self.values[i] intValue]) {
            res = [self.values[i] intValue];
        }
    }
    return res;
}
